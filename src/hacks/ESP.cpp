    ...
    if (*healthbar != 0)
    {
        ...
        if (type == ENTITY_PLAYER || type == ENTITY_BUILDING)
        {
            ...
            if (GetCollide(ent))
            {
                ...
                {
                    draw::RectangleOutlined(min_x - 7 - 2, min_y, 7, max_y - min_y, border, 0.5f);
                    draw::Rectangle(min_x - 6 - 2, max_y - hbh - 1, 5, hbh, hp);
                }
            }
        }
    }
    ...
    if (ent_data.string_count)
    {
        ...
        if (type == ENTITY_PLAYER || type == ENTITY_BUILDING)
        {
            ...
            if (GetCollide(ent))
            {
                ...
                case 0:
                { // TOP RIGHT
                    draw_point = Vector(max_x + 2 + 2, min_y, 0);
                }
                break;
                case 1:
                { // BOTTOM RIGHT
                    draw_point = Vector(max_x + 2 + 2, max_y - data.at(ent->m_IDX).string_count * 16, 0);
                }
                break;
                case 2:
                {                          // CENTER
                ...
                case 3:
                { // ABOVE CENTER
                    draw_point = Vector((min_x + max_x) / 2.0f, min_y - data.at(ent->m_IDX).string_count * 16 - 2, 0);
                }
                break;
                case 4:
                { // BELOW
                    draw_point = Vector((min_x + max_x) / 2.0f, max_y + 2, 0);
                }
                break;
                case 5:
                { // ABOVE LEFT
                    draw_point = Vector(min_x + 2, min_y - data.at(ent->m_IDX).string_count * 16 - 2, 0);
                }
                break;
                case 6:
                { // ABOVE RIGHT
                    draw_point = Vector(max_x + 2, min_y - data.at(ent->m_IDX).string_count * 16 - 2, 0);
                }
                }
            }
        }
...
void _FASTCALL DrawBox(CachedEntity *ent, const rgba_t &clr)
{
    ...
    else
    {
		...
		draw::RectangleOutlined(min_x + 1, min_y + 1, max_x - min_x - 2, max_y - min_y - 2, border, 4);
        draw::RectangleOutlined(min_x + 1, min_y + 1, max_x - min_x - 2, max_y - min_y - 2, clr, 2);
    }
}
